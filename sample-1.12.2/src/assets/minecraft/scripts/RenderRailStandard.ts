...
function init(par1: ModelSetRail, par2: ModelObject) {
...
function renderRailStatic(
  ...
  //MCP->SRGもうまくいく
  const x = tileEntity.getX();
  const y = tileEntity.getY();
  const z = tileEntity.getZ();
  const array = [x, y, z];

  for (const objNamesKey of tongBR.objNames) {
  }
}
...
function shouldRenderObject(
  ...
  if (renderer.isSwitchRail(tileEntity)) //分岐レール
  {
    ...
    return (
      staticParts.containsName(objName) ||
